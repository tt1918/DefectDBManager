...
//	m_bIsFocused = (lpDIS->itemState & ODS_FOCUS);
	...
	if (m_bDrawTransparent)
		...
		OnDrawBackground(memDC, &itemRect);

	// Draw button border
	OnDrawBorder(memDC, &itemRect);
...
{
	if ( NULL != m_UserFont ) pDC->SelectObject(m_UserFont);
	...
	pDC->DrawText(lpszText, -1, rpCaption, DT_WORDBREAK | DT_CALCRECT | m_TextAlign );
	...
	*/
	if (m_bIsDisabled)
	{
		...
		pDC->DrawText(lpszText, -1, rpCaption, DT_WORDBREAK | m_TextAlign);
		rpCaption->OffsetRect(-1, -1);
...
DWORD CButtonST::SetSound(LPCTSTR lpszSound, HMODULE hMod, BOOL bPlayOnClick, BOOL bPlayAsync)
{
	...
	if (lpszSound)
	{
		...
		{
			_tcscpy_s(m_csSounds[byIndex].szSound, strlen(lpszSound), lpszSound);
